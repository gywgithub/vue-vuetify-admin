<template>
  <div>
    ...
    <v-card class="my-2">
      ...
      <v-card-text>
        <v-tooltip bottom v-for="(item, k) in colorIcons" :key="k">
          <template v-slot:activator="{ on }">
            <svg class="icon" aria-hidden="true" v-on="on" :style="{color:item.color}">
    ...
    <v-card class="my-2">
      ...
      <v-card-subtitle>
        <a
          href="https://www.iconfont.cn/collections/detail?spm=a313x.7781069.1998910419.d9df05512&cid=16957"
          target="_blank"
        >https://www.iconfont.cn/collections/detail?spm=a313x.7781069.1998910419.d9df05512&cid=16957</a>
      </v-card-subtitle>
      <v-card-text>
        <v-tooltip bottom v-for="(item, k) in monochromeIcons" :key="k">
          <template v-slot:activator="{ on }">
            <svg class="icon" aria-hidden="true" v-on="on">
              <use :xlink:href="`#${item}`" />
            </svg>
...
export default class MaterialDesignIcons extends Vue {
  private multicolorIcons: string[] = [
    'icon-bangbangtang',
    'icon-aixin1',
    'icon-yinliao',
    'icon-yuyue',
    'icon-shengchanzhizuo',
    'icon-baosun',
    'icon-tuandui1',
    'icon-icon-test1',
    'icon-meirong1',
  ...
  private monochromeIcons: string[] = [
    'icon-aixin',
    'icon-bianji',
    'icon-Dyanjing',
    'icon-caidan',
